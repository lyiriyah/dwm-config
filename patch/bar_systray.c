...
int draw_systray(Bar *bar, BarArg *a) {
  if (!showsystray)
    return 0;
  ...
  if (!systray) {
    ...
    systray->win = XCreateSimpleWindow(dpy, root, bar->bx + a->x + lrpad / 2,
                                       bar->by + a->y + (a->h - systray->h) / 2,
                                       MIN(a->w, 1), systray->h, 0, 0,
  ...
  XMoveResizeWindow(
      dpy, systray->win, bar->bx + a->x + lrpad / 2,
      (w ? bar->by + a->y + (a->h - systray->h) / 2 : -bar->by - a->y),
      MAX(w, 1), systray->h);
  return w;
}

int click_systray(Bar *bar, Arg *arg, BarArg *a) { return -1; }
